...
void Box::init()
{
  ...
  static const std::array<textcoord_t, 4> textcoords = {
    textcoord_t{ 0.f, 1.f },  // top left
    textcoord_t{ 1.f, 1.f },  // top right
    textcoord_t{ 0.f, 0.f },  // bottom left
    textcoord_t{ 0.f, 1.f },  // bottom right
  };
  ...
  m_vbOBJ = std::unique_ptr<Buffer<GL_ARRAY_BUFFER>>(
    new Buffer<GL_ARRAY_BUFFER>(sizeof(position_t) + sizeof(color8_t) + sizeof(textcoord_t)));
  ...
  // init vertex buffer 
  m_vbOBJ.get()->namedBufferSubData(0, sizeof(position_t), localspace.data());
  m_vbOBJ.get()->namedBufferSubData(sizeof(position_t), sizeof(color8_t), colors.data());
  m_vbOBJ.get()->namedBufferSubData(sizeof(position_t)+sizeof(color8_t), sizeof(textcoord_t), textcoords.data());
  ...
  m_vaOBJ.get()->setAttribFormat(1, 3, GL_UNSIGNED_BYTE, true, sizeof(position_t));
  m_vaOBJ.get()->setAttribFormat(1, 2, GL_FLOAT, false, sizeof(position_t) + sizeof(color8_t));
